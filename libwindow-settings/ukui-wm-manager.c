...
void ukui_wm_manager_init(void)
{
	...
	tempdir = g_build_filename(UKUI_WM_PROPERTY_PATH, NULL);
	scan_wm_directory(tempdir, FALSE);
	g_free(tempdir);

		tempdir = g_build_filename(g_get_user_config_dir(), "ukui", "wm-properties", NULL);
...
UkuiWindowManager*
ukui_wm_manager_get_current (GdkScreen *screen)
{
        AvailableWindowManager *wm;
        ...
        if (wm != NULL && wm->module != NULL)
                ...
                return (UkuiWindowManager*) ukui_window_manager_new (wm->ditem);
        else
                return NULL;
}

gboolean
ukui_wm_manager_spawn_config_tool_for_current (GdkScreen  *screen,
                                                GError    **error)
{
        AvailableWindowManager *wm;
