...
1）确认添加后，再添加该快捷键会显示“快捷键冲突”；

2）鼠标悬浮在该快捷键上时，会出现删除按钮，若不再需要该快捷键，可点击删除。
...
- 选择输出设备：输出设备是获取系统可用的输出端口，可根据需要切换的对应的输出设备

- 声道平衡：调节输出音量的左右声道

- 选择输入设备：输入设备是获取系统可用的输入端口，可根据需要切换的对应的输入设备（输入设备主要用来录音视频以及通话）
...
![图 44 帐户信息设置-big](image/account.png)
...
![图 50 云帐户-big](image/cloudaccount.png)
### 登录方式
