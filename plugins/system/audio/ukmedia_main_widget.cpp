...
void UkmediaMainWidget::initGsettings()
{
    ...
    if (QGSettings::isSchemaInstalled(UKUI_SWITCH_SETTING)) {
        ...
        if (m_pBootSetting->keys().contains("logoutMusic")) {
            bool logout = m_pBootSetting->get(UKUI_LOGOUT_MUSIC_KEY).toBool();//注销音乐
            m_pSoundWidget->m_pLogoutButton->setChecked(logout);
        }
        if (m_pBootSetting->keys().contains("weakupMusic")) {
            bool m_hasMusic = m_pBootSetting->get(UKUI_WAKEUP_MUSIC_KEY).toBool();//休眠音乐
...
void UkmediaMainWidget::dealSlot()
{
    ...
    connect(m_pVolumeControl,&UkmediaVolumeControl::updateVolume,this,[=](int value,bool state){
        qDebug() << "Sink Volume Changed" << paVolumeToValue(value) << "muteStatus:" << state;
    ...
    connect(m_pVolumeControl,&UkmediaVolumeControl::updateSourceVolume,this,[=](int value,bool state){
        ...
        m_pInputWidget->m_pDnsNoiseReductionButton->isChecked();
        if(status){
            ...
            m_pInputWidget->m_pIpVolumeSlider->setValue(100);
        ...
        if(strstr(m_pVolumeControl->defaultSourceName,"monitor")){
            m_pInputWidget->m_pInputLevelProgressBar->blockSignals(true);
    ...
    connect(m_pInputWidget->m_pInputDeviceSelectBox,SIGNAL(currentIndexChanged(int)),this,SLOT(cboxinputListWidgetCurrentRowChangedSlot(int)));
}
...
//    qDebug() << "themeChangeIcons" << nInputValue << inputStatus;
    ...
    m_pSoundWidget->m_pAlertSoundMuteLabel->repaint();
    m_pSoundWidget->m_pAlertSoundVolumeLabel->repaint();
...
QPixmap UkmediaMainWidget::filledSymbolicColoredPixmap(QImage &img, QColor &baseColor)
{
...
void UkmediaMainWidget::startupButtonSwitchChangedSlot(bool status)
{
    ...
    qDebug() << "startupButtonSwitchChangedSlot---"<< status;
}
...
void UkmediaMainWidget::alertSoundButtonSwitchChangedSlot(bool status)
{
    ...
    qDebug() << "alertSoundButtonSwitchChangedSlot---" << status;
}

void UkmediaMainWidget::switchNoiseReductionButton(bool status)
{
    if (status) {
        system("pacmd switch-on-dns-global inteldns_source 1");
        m_pInputWidget->m_pIpVolumeSlider->setValue(paVolumeToValue(DNS_NOISE_REDUCTION_VOLUME));
        m_pInputWidget->m_pIpVolumeSlider->setEnabled(false);
        QString percent = QString::number(paVolumeToValue(DNS_NOISE_REDUCTION_VOLUME));
        m_pInputWidget->m_pIpVolumePercentLabel->setText(percent+"%");
...
void UkmediaMainWidget::dnsNoiseReductionButtonSwitchChangedSlot(bool status)
{
    switchNoiseReductionButton(status);
...
void UkmediaMainWidget::outputVolumeDarkThemeImage(int value,bool status)
{
    ...
    //调节提示音大小窗口两个图标的更新
    m_pSoundWidget->m_pAlertSoundMuteLabel->mColor= color;
    m_pSoundWidget->m_pAlertSoundVolumeLabel->mColor = color;
    m_pSoundWidget->m_pAlertSoundMuteLabel->mImage = QIcon::fromTheme("audio-volume-muted-symbolic").pixmap(24,24).toImage();
    m_pSoundWidget->m_pAlertSoundVolumeLabel->mImage = QIcon::fromTheme("audio-volume-high-symbolic").pixmap(24,24).toImage();
...
void UkmediaMainWidget::alertSoundSliderChangedSlot(int v)
{
    ...
    qDebug() << "alertSoundSliderChangedSlot---" << v;
}
        ...
        if (it.key() == cardIndex) {
            ...
            for (tempMap=portNameMap.begin();tempMap!=portNameMap.end();) {
                if (tempMap.value() == portName) {
                    if(!tempMap.key().contains("headset_head_unit"))
                        sinkStr = tempMap.key();
...
QString UkmediaMainWidget::findPortSource(int cardIndex,QString portName)
{
    ...
    for (it=m_pVolumeControl->sourcePortMap.begin();it!=m_pVolumeControl->sourcePortMap.end();) {
        if(it.key() == cardIndex) {
            ...
            for (tempMap=portNameMap.begin();tempMap!=portNameMap.end();) {
                if (tempMap.value() == portName) {
                    if(!tempMap.key().contains("a2dp_source"))
...
void UkmediaMainWidget::findOutputComboboxItem(QString cardName,QString portLabel)
{
    for (int row=0;row<m_pOutputWidget->m_pDeviceSelectBox->count();row++) {
        ...
        if (comboboxcardname == cardName && comboboxportname == portLabel) {
            ...
            qDebug() << "findOutputListWidgetItem" << "card name:" << cardName << "port name:" << portLabel;
            break;
        }
    }
 ...
 */
void UkmediaMainWidget::addComboboxOutputListWidgetItem(QString portName, QString cardName)
{
    int i = m_pOutputWidget->m_pDeviceSelectBox->count();
    m_pOutputWidget->m_pDeviceSelectBox->blockSignals(true);
...
void UkmediaMainWidget::findInputComboboxItem(QString cardName,QString portLabel)
{
    for (int row=0;row<m_pInputWidget->m_pInputDeviceSelectBox->count();row++) {
        ...
        if (comboboxcardname == cardName && comboboxportname == portLabel) {
            ...
            qDebug() << "findInputComboboxItem" << "card name:" << cardName << "port name:" << portLabel;
            break;
        }
    }
}
 ...
 */
void UkmediaMainWidget::addComboboxInputListWidgetItem(QString portName, QString cardName)
{
 ...
 */
void UkmediaMainWidget::cboxoutputListWidgetCurrentRowChangedSlot(int row)
{
    //当所有可用的输出设备全部移除，台式机才会出现该情况
    if (row == -1)
        return;

    QString outputComboboxPortName = m_pOutputWidget->m_pDeviceSelectBox->itemText(row);
    QString outputComboboxCardName = m_pOutputWidget->m_pDeviceSelectBox->itemData(row).toString();
    ...
    if (isContainBlue && (strstr(m_pVolumeControl->defaultSourceName,\
        "headset_head_unit") || strstr(m_pVolumeControl->defaultSourceName,"bt_sco_sink")))
    {
        ...
        if(currentCardIndex == outputProfileMap.key()){
            ...
            for(at=tempMap.begin(),i=0;at!= tempMap.end();++i){
                if (at.key() == outputComboboxPortNameList.at(0)) {
                    ...
                    if(endOutputProfile == "headset_head_unit")
...
void UkmediaMainWidget::cboxinputListWidgetCurrentRowChangedSlot(int row)
{
    ...
    QString inputComboboxPortName = m_pInputWidget->m_pInputDeviceSelectBox->itemText(row);
    QString inputComboboxCardName = m_pInputWidget->m_pInputDeviceSelectBox->itemData(row).toString();
