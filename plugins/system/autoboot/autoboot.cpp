...
bool AutoBoot::eventFilter(QObject *w, QEvent *e){
    if (w == ui->editLabel){
        if(e->type() == QEvent::MouseButtonPress){
            if(!isEdit){
                isEdit = true;
                ui->editLabel->setText(tr("Done"));
            } else {
                isEdit = false;
                ui->editLabel->setText(tr("Edit"));
            }
            clearAutoItem();
            initUI();
        }
...
QWidget *AutoBoot::get_plugin_ui(){
    if (mFirstLoad) {
        ...
        isEdit = false;
        ui->editLabel->setText(tr("Edit"));
        ui->editLabel->setCursor(QCursor(Qt::PointingHandCursor));
        ui->editLabel->setStyleSheet("QLabel{color: #2FB3E8;}");
        ui->editLabel->installEventFilter(this);
    ...
    //    ui->addFrame->installEventFilter(this);
        ...
        ui->titleLabel->setText(tr("App List"));
        ui->titleLabel2->setText(tr("Add Apps"));
        ui->editLabel->close();
        ui->titleLabel2->close();


        localconfigdir = g_build_filename(g_get_user_config_dir(), "autostart", NULL);
        qDebug()<<"localconfigdir"<<localconfigdir;
        initAddBtn();
        initUI();

        //初始化添加界面
        dialog = new AddAutoBoot(addedApps);
        ...
        for(int i=0;i<desktopfpvar.count();i++)
        {
            ...
            desktopfpList.append(QString("%1.desktop").arg(s));
            qDebug()<<"s1="<<desktopfpList.at(i);

        }

        QMap<QString, AutoApp>::iterator iter = statusMaps.begin();
        bool deletestatus = false;
        while (iter != statusMaps.end())
        {
            //qDebug() << "Iterator " << iter.key(); // 迭代器
            if (desktopfpList.indexOf(iter.key()) == -1) {
                QString s =iter.key();
                s.chop(8);
                QString st =iter.key();
                iter++;
                deletestatus = true;
                del_autoboot_realize(s,st);
            } else {
                iter++;
            }
         }
        ...
        qDebug()<<"num ="<<num;
        qDebug()<<"desk ="<<desktopfpList.count();
        bool addappstatus = false;
        for(int i=0;i <desktopfpList.count(); i++)
        {
            QMap<QString, AutoApp>::iterator it = statusMaps.find(desktopfpList.at(i));
            if (it == statusMaps.end()){
                QString s = "/usr/share/applications/";
                s = s+ desktopfpList.at(i);
                addappstatus = true;
                dialog->build_app_item(s);
...
//        button->setChecked(it.value().enable);
        ...
        dBtn->setFixedSize(QSize(24, 24));
//        dBtn->setText("Del");
        dBtn->setStyleSheet("QPushButton{border-radius: 12px; background-color: #FD625E;}"
                            "QPushButton:hover{background-color: #E6332D;}");
        dBtn->setHidden(true);
        QHBoxLayout * dBtnHorLayout = new QHBoxLayout(button);
        QLabel * dLine = new QLabel();
        dLine->setFixedSize(14, 2);
        dLine->setStyleSheet("QLabel{background-color: #FFFFFF; border-radius: 1px;}");
        dBtnHorLayout->addWidget(dLine);
        dBtnHorLayout->setContentsMargins(0,0,0,0);
        dBtn->setLayout(dBtnHorLayout);
        connect(dBtn, &QPushButton::clicked, this, [=]{
//            del_autoboot_realize(bname);
            DelAutoBoot * del_dialog = new DelAutoBoot(appName, bname, it.value().pixmap);
            connect(del_dialog, SIGNAL(autoboot_deleted(QString, QString)), this, SLOT(del_autoboot_realize(QString, QString)));
            del_dialog->exec();
        });
//        dBtn->setStyleSheet(""
...
//        ui->listWidget->setSpacing(1);
        if (it.value().xdg_position == LOCALPOS){
            ui->addAutoLayout->addWidget(baseWidget);
            if(!addedApps.contains(it.value().name)){
                addedApps.append(it.value().name);
            }
        } else {
...
//        delete ui->availableLayout->layout();
    ...
    if (ui->addAutoLayout->layout() != NULL) {
        QLayoutItem* item;
        while ((item = ui->addAutoLayout->layout()->takeAt(0)) != NULL )
        {
            delete item->widget();
            delete item;
        }
...
//    enable = _key_file_get_boolean(keyfile, APP_KEY_FILE_DESKTOP_KEY_AUTOSTART_ENABLE, TRUE);
    ...
    QString localpath = QString("%1%2").arg("/usr/share/applications/").arg(bname);
    QByteArray ba;
    ba = localpath.toUtf8();
    GKeyFile * icon_keyfile;
    icon_keyfile = g_key_file_new();
    if (!g_key_file_load_from_file(icon_keyfile, ba.data(), G_KEY_FILE_NONE, NULL)){
        icon = g_key_file_get_locale_string(keyfile, G_KEY_FILE_DESKTOP_GROUP, G_KEY_FILE_DESKTOP_KEY_ICON, NULL, NULL);
    } else {
        icon = g_key_file_get_locale_string(icon_keyfile, G_KEY_FILE_DESKTOP_GROUP, G_KEY_FILE_DESKTOP_KEY_ICON, NULL, NULL);
    }
...
void AutoBoot::add_autoboot_realize_slot(QString path, QString name, QString exec, QString comment){
    ...
    if(!addedApps.contains(name)){
        addedApps.append(name);
    }
...
void AutoBoot::del_autoboot_realize(QString name,QString bname){
    ...
    addedApps.removeOne(name);
//    dialog->refreshList(name);
