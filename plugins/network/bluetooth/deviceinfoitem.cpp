...
void DeviceInfoItem::DrawStatusText(QPainter &painter)
{
    ...
    {
        if (LINK == _DevStatus)
            painter.drawText(this->width()-210,16,150,24,Qt::AlignRight,tr(DEV_DISCONNECTING_TEXT));
        else
        {
            if (_MDev.data()->isPaired())
                painter.drawText(this->width()-210,18,150,24,Qt::AlignRight,tr(DEV_CONNECTING_TEXT));
            else
                painter.drawText(this->width()-210,18,180,24,Qt::AlignRight,tr(DEV_CONNECTING_TEXT));
        }
    }
    else if (DEVICE_STATUS::ERROR == _DevStatus)
    {
        if (_MDev.data()->isPaired())
            painter.drawText(this->width()-210,18,150,24,Qt::AlignRight,tr(DEV_CONNECTION_FAIL_TEXT));
        else
            painter.drawText(this->width()-210,18,180,24,Qt::AlignRight,tr(DEV_CONNECTION_FAIL_TEXT));
    }
    else if (DEVICE_STATUS::PAIRED == _DevStatus)
        painter.drawText(this->width()-210,18,150,24,Qt::AlignRight,tr(DEV_UNUNITED_TEXT));
    else if (DEVICE_STATUS::LINK == _DevStatus)
        painter.drawText(this->width()-210,18,150,24,Qt::AlignRight,tr(DEV_CONNECTED_TEXT));
    else
