    ...
    if (getWifiStatus() && reply.value().length() != 0 && reply.value().length() == 1 && wirelessStatus && getHasWirelessCard()) {
        ...
        if (getWifiListDone(reply, this->TlanList) == -1) {
            ...
            for (int i = 1; i < reply.value().length(); i++) {
                ...
                QString signal = reply.value().at(i).at(2);
            ...
            if (!replyDevicesPaths.isEmpty()) {
                if (!activeNet.strConType.compare("802-3-ethernet", Qt::CaseInsensitive)) {
                    QDBusInterface netDeviceifc("org.freedesktop.NetworkManager",
                                                ...
                                                QDBusConnection::systemBus());
                    activeNet.strBandWidth = netDeviceifc.property("Speed").toString() + "Mbps";
                    ...
                    QDBusInterface netDeviceifc("org.freedesktop.NetworkManager",
                                                ...
                                                QDBusConnection::systemBus());
                    float bandWidth = netDeviceifc.property("Bitrate").toFloat() / 1000;
                    activeNet.strBandWidth = QString("%1").arg(bandWidth) + "Mbps";
                    activeNet.strMac = netDeviceifc.property("HwAddress").toString().toLower();
                }
            } else {
