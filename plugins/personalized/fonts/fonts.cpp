...
QWidget *Fonts::get_plugin_ui(){
    if (mFirstLoad) {
        ...
        //隐藏字体大小设置
        ui->fontSizeFrame->hide();
        ui->fontTypeFrame_2->close();
        ...
        ui->fontSelectComBox->setCurrentText(sysfont);
...
void Fonts::initSearchText() {
    ////~ contents_path /fonts/Normal font
    ui->fontSelectLabel->setText(tr("Normal font"));
...
void Fonts::setupComponent(){
    ...
    fontScale<< tr("min") << tr("little") << tr("mid") << tr("large") << tr("max")
              <<tr("16") << tr("17") << tr("18");
    ...
    ui->fontLayout->addItem(new QSpacerItem(8,20,QSizePolicy::Fixed));
    ui->fontLayout->addWidget(uslider);
    ui->fontLayout->addItem(new QSpacerItem(20,20,QSizePolicy::Fixed));
    ui->fontLayout->addSpacing(6);

    //导入系统字体列表
    QStringList fontfamiles = fontdb.families();

    for (int i=0;i<fontfamiles.count();i++){
        if(fontfamiles.at(i)=="Noto Sans CJK SC") {
            fontfamiles.replace(i,fontfamiles.at(i)+tr("(default)"));
            break;
        }
    }

    for (int i=0;i<fontfamiles.count();i++){
        if(fontfamiles.at(i)=="DejaVu Sans Mono") {
            fontfamiles.replace(i,fontfamiles.at(i)+tr("(default)"));
            break;
        }
    }
    ...
    for (QString font : fontfamiles){
        ...
        //字体
        if (laungeBool) {
            if (font == "Noto Sans CJK SC(默认)"){
                ui->fontSelectComBox->addItem(font);
            }
        } else {
            if (font == "Noto Sans CJK SC(default)"){
                ui->fontSelectComBox->addItem(font);
            }
        }
        //等宽字体
...
//        peonysettings->set(PEONY_FONT_KEY, QVariant(QString("%1 %2").arg(text).arg(peonyfontStrList.at(1))));
    ...
    //重置按钮
//    ui->resetBtn->setLayout();
//    resetBtn = new HoverWidget("");
//    resetBtn->hide();
//    resetBtn->setObjectName("resetBgd");
//    resetBtn->setFixedSize(136,56);
//    resetBtn->setStyleSheet("HoverWidget#resetBgd{background: palette(base); border-radius: 12px;}"
//                            "HoverWidget:hover:!pressed#resetBgd{background: #2FB3E8; border-radius: 12px;}");
//    addLyt_2 = new QHBoxLayout;
//    QLabel * iconLabel_2 = new QLabel();
      ////~ contents_path /fonts/Reset
...
//    addLyt_2->addStretch();
//    resetBtn->setLayout(addLyt_2);
//    ui->horizontalLayout_4->addWidget(resetBtn);
//    ui->horizontalLayout_4->addItem(new QSpacerItem(16,20,QSizePolicy::Expanding));
//    // 悬浮改变Widget状态
//    connect(resetBtn, &HoverWidget::enterWidget, this, [=](QString mname){
//        Q_UNUSED(mname);
...
//    });
//    // 还原状态
//    connect(resetBtn, &HoverWidget::leaveWidget, this, [=](QString mname){
//        Q_UNUSED(mname);
//        QPixmap pixgray_2 = ImageUtil::loadSvg(":/img/titlebar/resetBgd.svg", "default", 24);
//        iconLabel_2->setPixmap(pixgray_2);
//        textLabel_2->setStyleSheet("color: #2FB3E8;font-weight: 400;");
//    });
//    connect(resetBtn, &HoverWidget::widgetClicked, [=]{
//        resetDefault();
//    });
}
...
void Fonts::initGeneralFontStatus(){
    ...
    ui->fontSelectComBox->blockSignals(true);
    ui->fontSelectComBox->setCurrentText(currentfonts+tr("(default)"));
    ui->fontSelectComBox->blockSignals(false);
    ...
    ui->monoSelectComBox->setCurrentText(currentmonofont+tr("(default)"));
