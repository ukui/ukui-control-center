...
void m_updatelog::slotSearch(QString packageName)
{
    ...
    /* 拼接sql */
    QString sql = "SELECT `appname` , `version` , `status` , `date` , `description` , `errorcode` , `id` , `keyword` FROM updateinfos WHERE `appname` = '" + dstPackageName + "'";
    ...
    while (query.next()) {
        ...
        QString errorcode = query.value(5).toString();
        int id = query.value(6).toInt();
        QString keyword = query.value(7).toString();
        ...
        updateItem->setAttribute(packageName + " " + version , status , time , description , errorcode , id);
        QListWidgetItem *item = new QListWidgetItem();
        item->setFlags(Qt::NoItemFlags);
...
//    QFont font;
//    font.setPointSize(14);//font.setPixelSize(14);从固定字体改变为可变化字体,这两行都不要了
