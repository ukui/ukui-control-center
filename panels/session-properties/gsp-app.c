...
const char *
gsp_app_get_icon_char (GspApp *app)
{
        g_return_val_if_fail (GSP_IS_APP (app), NULL);
                ...
                const char *icon)
{
        ...
        app->priv->path = g_build_filename (g_get_user_config_dir (),
                                            ...
                                            app->priv->basename, NULL);
        manager = gsp_app_manager_get ();
        if(NULL != gsp_app_manager_find_local_app_with_name_exec(manager,name,exec,icon))
        {
            return 8;
        }
        ...
        if(icon==NULL)
        {
            ...
            app->priv->gicon = g_icon_new_for_string (icon,NULL);
        }
