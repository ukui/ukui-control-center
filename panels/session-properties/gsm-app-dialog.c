...
static void get_desktop_file_info(GsmAppDialog *dialog, const char * filename){
    GDesktopAppInfo * app_info;
    char * name;
    char * description;
	char * commandline;
    char * icon;
    //analysis desktop file,we use GDesktopAppInfo, and then translates to G_APP_INFO,because
    //GDesktopAppInfo get description is not locale string,g_app_info* use g_key_file* to get string in desktop
    //we can read the code in glib--gio
	...
	commandline = g_app_info_get_commandline(G_APP_INFO(app_info));
    icon = g_app_info_get_icon(app_info);
    if(icon)
        dialog->priv->icon = g_icon_to_string(icon);
    else
        dialog->priv->icon = NULL;
    gtk_entry_set_text(GTK_ENTRY(dialog->priv->name_entry), name);
    gtk_entry_set_text(GTK_ENTRY(dialog->priv->comment_entry), description);
	gtk_entry_set_text(GTK_ENTRY(dialog->priv->command_entry), commandline);
                          ...
                          GsmAppDialog *dialog)
{
        ...
        //                              GTK_WINDOW (dialog));
		gtk_file_chooser_set_current_folder(GTK_FILE_CHOOSER(chooser),"/etc/xdg/autostart");

		filter = gtk_file_filter_new();
        gtk_file_filter_set_name(filter,_("Desktop file(*.desktop)"));
        ...
        if (response == GTK_RESPONSE_ACCEPT) {
                ...
                get_desktop_file_info(dialog,text);
...
setup_dialog (GsmAppDialog *dialog)
{
        ...
        if (!gtk_builder_add_from_file (xml,
                ...
                if (gtk_builder_add_from_file(xml, "../panels/session-properties", NULL)==0){
	...
	gtk_window_set_skip_taskbar_hint(GTK_WINDOW(dialog), TRUE);
        ...
        gtk_window_set_icon_name (GTK_WINDOW (dialog), "ukui-session-properties");
        ...
        if (dialog->priv->name == NULL
            ...
            && dialog->priv->comment == NULL) {
                gtk_window_set_title (GTK_WINDOW (dialog), _("Add programs"));
                gtk_dialog_add_button (GTK_DIALOG (dialog),
                                       GTK_STOCK_ADD, GTK_RESPONSE_OK);
        } else {
                gtk_window_set_title (GTK_WINDOW (dialog), _("Boot program property details"));
                gtk_editable_set_editable(GTK_EDITABLE(dialog->priv->command_entry), FALSE);
                gtk_editable_set_editable(GTK_EDITABLE(dialog->priv->name_entry), FALSE);
                gtk_editable_set_editable(GTK_EDITABLE(dialog->priv->comment_entry), FALSE);
                gtk_widget_set_sensitive(dialog->priv->browse_button, FALSE);
                //gtk_dialog_add_button (GTK_DIALOG (dialog),
                    ...
                    char         **icon_p)
{
        ...
        while (gtk_dialog_run (GTK_DIALOG (dialog)) == GTK_RESPONSE_OK) {
                ...
                if(icon_p)
                {
                    if(dialog->priv->icon!=NULL)
                         *icon_p = g_strdup (dialog->priv->icon);
                    else
                         *icon_p = g_strdup ("system-run");
                }
                error = NULL;
                error_msg = NULL;
