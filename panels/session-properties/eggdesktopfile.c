...
EggDesktopFile* egg_desktop_file_new_from_key_file(GKeyFile* key_file, const char* source, GError** error)
{
    ...
    {
        g_set_error(error, EGG_DESKTOP_FILE_ERROR, EGG_DESKTOP_FILE_ERROR_INVALID, "File is not a valid .desktop file");
        g_key_file_free(key_file);
        return NULL;
    }
    ...
    if (version)
    {
        ...
        {
            g_set_error(error, EGG_DESKTOP_FILE_ERROR, EGG_DESKTOP_FILE_ERROR_INVALID, "Unrecognized desktop file Version '%s'", version);
            g_free(version);
            g_key_file_free(key_file);
            return NULL;
        }
                ...
                guint32         launch_time)
{
  ...
  description = g_strdup_printf ("Starting %s", desktop_file->name);
