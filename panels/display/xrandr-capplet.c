...
add_key (GtkWidget *widget,
     const char *text,MateRROutputInfo *output,
...
rebuild_rotation_combo (App *app)
{
	...
	{
        add_key (app->rotation_combo, _(info->name),NULL, 0, 0, 0, info->rotation);
...
rebuild_current_monitor_label (App *app)
{
	...
	gtk_label_set_text(GTK_LABEL (app->current_monitor_label), "11111111111111");
	if (use_color)
	{
#if GTK_CHECK_VERSION (3, 0, 0)
...
static void
rebuild_display_combo (App *app)
{
    int i;
    int monitor_num=0;
    char *current_display_name;
    clear_combo (app->dispaly_combo);
    ...
    for (i = 0; outputs[i] != NULL; ++i)
    {
        ...
        {
//                g_printerr("-----------display_name=%s\n",g_strdup_printf ("<b>%s</b>\n<small>%s</small>", mate_rr_output_info_get_display_name (output), mate_rr_output_info_get_name (output)));
            add_key (app->dispaly_combo,g_strdup_printf ("%s<%s>", mate_rr_output_info_get_display_name (output), mate_rr_output_info_get_name (output)),output, 0, 0, 0, -1);
                monitor_num++;
        }
    ...
    if (app->current_output)
    {
        current_display_name= g_strdup_printf ("%s<%s>", mate_rr_output_info_get_display_name (app->current_output), mate_rr_output_info_get_name (app->current_output));
        combo_select (app->dispaly_combo, current_display_name);
    }
    if (monitor_num > 1)
          gtk_widget_set_sensitive (app->dispaly_combo, TRUE);
    else
          gtk_widget_set_sensitive (app->dispaly_combo, FALSE);
}
...
rebuild_resolution_combo (App *app)
{
	...
	add_key (app->resolution_combo,
         idle_free (make_resolution_string (width, height)),app->current_output,
		 width, height, 0, -1);
    }
...
static gboolean
get_mode (GtkWidget *widget, char** text,MateRROutputInfo **output,int *width, int *height, int *freq, MateRRRotation *rot)
{
    GtkTreeIter iter;
    GtkTreeModel *model;
    GtkComboBox *box = GTK_COMBO_BOX (widget);
...
on_rotation_changed (GtkComboBox *box, gpointer data)
{
    ...
    if (get_mode (app->rotation_combo, NULL,NULL,NULL, NULL, NULL, &rotation))
...
static void
on_display_changed (GtkComboBox *box, gpointer data)
{
    App *app = data;
    char* display_name;
    MateRROutputInfo *output=NULL;

    if (!app->current_output)
        return;

    if (get_mode (app->dispaly_combo, &display_name,&output, NULL, NULL,NULL,NULL))
        if(output)
        {
...
on_resolution_changed (GtkComboBox *box, gpointer data)
{
    ...
    if (get_mode (app->resolution_combo,NULL,NULL, &width, &height, NULL, NULL))
    {
	mate_rr_output_info_set_geometry (app->current_output, x, y, width, height);
...
static gboolean
dialog_realize_event_cb (GtkWidget *widget, GdkEventAny *event, gpointer data)
{
    App *app = data;
...
apply_button_clicked_cb (GtkButton *button, gpointer data)
{
    ...
    if(abs(backup_timestamp-rawtime)>=7)//7s内连续点击无效
    {
    	backup_timestamp = rawtime;
...
static int
run_application (App *app, GtkBuilder *builder)
{
    GtkWidget *align;
    GError *error;
    error = NULL;
    ...
    app->dialog = _gtk_builder_get_widget (builder, "window1");
    g_signal_connect_after (app->dialog, "map-event",
			    G_CALLBACK (dialog_map_event_cb), app);
    /*g_signal_connect (app->dialog, "realize",
			    G_CALLBACK (dialog_realize_event_cb), app);*/
    ...
    app->dispaly_combo = _gtk_builder_get_widget(builder,"combobox_display");
    g_signal_connect (app->dispaly_combo, "changed",
                      G_CALLBACK (on_display_changed), app);
...
//		      G_CALLBACK (on_rate_changed), app);
    ...
    app->clone_checkbox = _gtk_builder_get_widget (builder, "checkbutton_display_clone");
...
//    make_text_combo (app->refresh_combo, 3);
    ...
    make_text_combo (app->dispaly_combo, 2);
//    g_assert (app->panel_checkbox);
    ...
    align = _gtk_builder_get_widget (builder, "alignment_display");
    ...
    app->apply_button = _gtk_builder_get_widget (builder, "button_keep_dispaly_setting");
