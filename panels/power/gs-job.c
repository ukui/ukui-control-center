...
widget_get_id_string (GtkWidget *widget)
{
    ...
    if(win == NULL)
        id = g_strdup_printf ("0x%X", (guint32)gtk_widget_get_window (widget));
                              //(guint32)GDK_WINDOW_XID (gtk_widget_get_window (widget)));
    else
        id = g_strdup_printf ("0x%X", //GDK_WINDOW_XID(gtk_widget_get_window (widget)));
                          (guint32)GDK_WINDOW_XID (gtk_widget_get_window (widget)));
	return id;
}
...
wait_again:
	if (waitpid (pid, &status, 0) < 0)
	{
		...
		{
            g_debug ("waitpid () should not fail in 'GSJob'");
		}
	}
...
gs_job_died (GSJob *job)
{
	if (job->priv->pid > 0)
	{
        ...
        g_debug ("Waiting on process %d", job->priv->pid);
                     ...
                     const char *command)
{
    ...
    g_debug ("Setting command for job: '%s'",
...
#if defined(HAVE_SETPRIORITY) && defined(PRIO_PROCESS)
    g_debug ("Setting child process priority to: %d", nice_level);
	if (setpriority (PRIO_PROCESS, pid, nice_level) != 0)
	{
        g_debug ("setpriority(PRIO_PROCESS, %lu, %d) failed",
		          (unsigned long) pid, nice_level);
	}
#else
    g_debug ("don't know how to change process priority on this system.");
#endif
}
                 ...
                 guint      *watch_id)
{
	...
	{
        g_debug ("Could not parse command: %s", error->message);
		g_error_free (error);
		return FALSE;
	}
	...
	if (! result)
	{
        g_debug ("Could not start command '%s': %s", command, error->message);
		g_error_free (error);
		g_strfreev (argv);
		return FALSE;
	}
               ...
               GSJob        *job)
{
	...
	if (condition & G_IO_IN)
	{
		...
		{
            g_debug ("command error: %s", error->message);
			g_error_free (error);
		}
...
gs_job_start (GSJob *job)
{
	...
	if (job->priv->pid != 0)
	{
        g_debug ("Cannot restart active job.");
		return FALSE;
	}

	if (job->priv->widget == NULL)
	{
        g_debug ("Could not start job: screensaver window is not set.");
		return FALSE;
	}
	...
	if (job->priv->command == NULL)
	{
		/* no warning here because a NULL command is interpreted
		   as a no-op job */
        g_debug ("No command set for job.");
		return FALSE;
	}
...
gs_job_stop (GSJob *job)
{
    ...
    g_debug ("stopping job");

	if (job->priv->pid == 0)
	{
        g_debug ("Could not stop job: pid not defined");
		return FALSE;
	}
