...
kpm_brightness_helper_get_value (const gchar *argument)
{
	...
	/* get the data */
	command = g_strdup_printf (SBINDIR "/ukui-power-backlight-helper --%s", argument);
...
kpm_brightness_helper_set_value (const gchar *argument, gint value)
{
	...
	/* get the data */
	command = g_strdup_printf ("pkexec " SBINDIR "/ukui-power-backlight-helper --%s %i", argument, value);
