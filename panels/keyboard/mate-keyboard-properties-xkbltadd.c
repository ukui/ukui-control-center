...
static void
xkb_layout_chooser_add_variant_to_available_country_variants(XklConfigRegistry * config_registry,
     							      ...
     							      AddVariantData * data) 
{
	gchar *utf_variant_name = config_item ?
	    xkb_layout_description_utf8 (matekbd_keyboard_config_merge_items
					 ...
					 config_item->name)) : xci_desc_to_utf8 (parent_config_item);
	GtkTreeIter iter;
	const gchar *xkb_id =
	    config_item ?
	    matekbd_keyboard_config_merge_items (parent_config_item->name,
					      config_item->name) : parent_config_item->name;
...
static void
xkb_layout_chooser_add_variant_to_available_language_variants(XklConfigRegistry * config_registry,
     							      XklConfigItem * parent_config_item, XklConfigItem * config_item,
     							      AddVariantData * data) 
{
	xkb_layout_chooser_add_variant_to_available_country_variants(config_registry, 
								     parent_config_item, 
								     config_item, 
								     data);
}
...
{
	xkb_layout_chooser_available_language_variants_fill (chooser_dialog);
...
{
	xkb_layout_chooser_available_country_variants_fill (chooser_dialog);
	xkb_layout_chooser_available_variant_changed (chooser_dialog);
}
						     ...
						     chooser_dialog)
{
	...
	if (gtk_combo_box_get_active_iter (GTK_COMBO_BOX (cbl), &liter)) 
	{
		...
		xkl_config_registry_foreach_language_variant (config_registry, 
							      lang_id, 
							      (TwoConfigItemsProcessFunc)xkb_layout_chooser_add_variant_to_available_language_variants,
		     					      &data);
		g_free (lang_id);
	}
						    ...
						    chooser_dialog)
{
	...
	if (gtk_combo_box_get_active_iter (GTK_COMBO_BOX (cbl), &liter)) 
	{
		...
		gtk_tree_model_get (lm, &liter,
				    COMBO_BOX_MODEL_COL_REAL_ID,
				    &country_id, -1);
		xkl_config_registry_foreach_country_variant (config_registry, 
							     country_id,
							     (TwoConfigItemsProcessFunc)xkb_layout_chooser_add_variant_to_available_country_variants,
							     &data);
		g_free (country_id);
	}
...
xkb_layout_chooser_print (GtkBuilder * chooser_dialog)
{
	...
	matekbd_keyboard_drawing_print (MATEKBD_KEYBOARD_DRAWING(kbdraw),
				        GTK_WINDOW (CWID("xkb_layout_chooser")),
				        descr);
	g_free (descr);
}
			     ...
			     gint response, GtkBuilder * chooser_dialog)
{
	...
	if (response == GTK_RESPONSE_OK) {
		...
		if (selected_id != NULL) {
			...
			layouts_list = g_slist_append (layouts_list, selected_id);
			xkb_layouts_set_selected_list (layouts_list);

			xkl_layout_chooser_add_default_switcher_if_necessary (layouts_list);

			clear_xkb_elements_list (layouts_list);
		}
	} 
	else if (response == gtk_dialog_get_response_for_widget (dialog, CWID ("btnPrint"))) 
	{
...
xkb_layout_choose (GtkBuilder * dialog)
{
	...
	xkb_layout_chooser_available_layouts_fill (chooser_dialog,
						   ...
						   G_CALLBACK(xkb_layout_chooser_available_country_changed));
	xkb_layout_chooser_available_layouts_fill (chooser_dialog,
						   ...
						   G_CALLBACK(xkb_layout_chooser_available_language_changed));
	...
	if (gtk_tree_model_iter_n_children
	    (gtk_combo_box_get_model (GTK_COMBO_BOX (lang_chooser)), NULL)) 
	{
		gtk_combo_box_set_active (GTK_COMBO_BOX
					  (CWID("xkb_languages_available")),
					  10);
	} 
	else 
	{
...
{
	GtkWidget *cbv = CWID (gtk_notebook_get_current_page
		  	       (GTK_NOTEBOOK (CWID ("choosers_nb"))) ?
		  	       "xkb_language_variants_available" :
		  	       "xkb_country_variants_available");
