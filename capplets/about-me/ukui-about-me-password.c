 ...
 * 02110-1301, USA.
 *
 * Parts of this code come from Ukui-System-Tools.
 */
 ...
 * Error handling {{
 */
#define PASSDLG_ERROR (ukui_about_me_password_error_quark())

GQuark ukui_about_me_password_error_quark(void)
{
	static GQuark q = 0;

	if (q == 0) {
		q = g_quark_from_static_string("ukui_about_me_password_error");
	}
...
passdlg_init (PasswordDialog *pdialog, GtkWindow *parent)
{
    ...
    gtk_builder_add_from_file (dialog, UKUICC_UI_DIR "/ukui-about-me-password.ui", NULL);
	pdialog->ui = dialog;
...
/* Main */
void
ukui_about_me_password (GtkWindow *parent)
{
