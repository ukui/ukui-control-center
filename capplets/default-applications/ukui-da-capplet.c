...
static void
close_cb(GtkWidget* window, gint response, UkuiDACapplet* capplet)
{
...
static void
document_combo_changed_cb(GtkComboBox* combo, UkuiDACapplet* capplet)
{
	set_changed(combo, capplet, capplet->document_viewers, DA_TYPE_DOCUMENT);
}

static void
word_combo_changed_cb(GtkComboBox* combo, UkuiDACapplet* capplet)
{
	set_changed(combo, capplet, capplet->word_editors, DA_TYPE_WORD);
}

static void
spreadsheet_combo_changed_cb(GtkComboBox* combo, UkuiDACapplet* capplet)
{
...
static void
theme_changed_cb(GtkIconTheme* theme, UkuiDACapplet* capplet)
{
	GObject* icon;
	gint i;
...
static void
screen_changed_cb(GtkWidget* widget, GdkScreen* screen, UkuiDACapplet* capplet)
{
	GtkIconTheme* theme;
...
static void
show_dialog(UkuiDACapplet* capplet, const gchar* start_page)
{
	...
	if (g_file_test(UKUICC_UI_DIR "/ukui-default-applications-properties.ui", G_FILE_TEST_EXISTS) != FALSE)
	{
		builder_result = gtk_builder_add_from_file(builder, UKUICC_UI_DIR "/ukui-default-applications-properties.ui", NULL);
	}
	else
	{
		builder_result = gtk_builder_add_from_file(builder, "./ukui-default-applications-properties.ui", NULL);
	}
...
main(int argc, char** argv)
{
	...
	GOptionContext* context = g_option_context_new(_("- UKUI Default Applications"));
	g_option_context_add_main_entries(context, option_entries, GETTEXT_PACKAGE);

	capplet_init(context, &argc, &argv);

	UkuiDACapplet* capplet = g_new0(UkuiDACapplet, 1);
