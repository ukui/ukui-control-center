...
#include "ukui-metacity-support.h"
#include "wm-common.h"
...
#define UKWM_SIDE_BY_SIDE_TILING_KEY "side-by-side-tiling"
#define UKWM_CENTER_NEW_WINDOWS_KEY "center-new-windows"
#define UKWM_BUTTON_LAYOUT_KEY "button-layout"

#define UKWM_BUTTON_LAYOUT_RIGHT "menu:minimize,maximize,close"
#define UKWM_BUTTON_LAYOUT_LEFT "close,minimize,maximize:"
...
static GSettings *ukwm_settings;
...
update_sensitivity ()
{
    ...
    gtk_widget_set_sensitive (GTK_WIDGET (compositing_fast_alt_tab_checkbutton),
                              g_settings_get_boolean (ukwm_settings, UKWM_COMPOSITING_MANAGER_KEY));
    gtk_widget_set_sensitive (GTK_WIDGET (focus_mode_mouse_checkbutton),
                              g_settings_get_enum (ukwm_settings, UKWM_FOCUS_KEY) != FOCUS_MODE_CLICK);
    gtk_widget_set_sensitive (GTK_WIDGET (autoraise_checkbutton),
                              g_settings_get_enum (ukwm_settings, UKWM_FOCUS_KEY) != FOCUS_MODE_CLICK);
    gtk_widget_set_sensitive (GTK_WIDGET (autoraise_delay_hbox),
                              g_settings_get_enum (ukwm_settings, UKWM_FOCUS_KEY) != FOCUS_MODE_CLICK &&
                              g_settings_get_boolean (ukwm_settings, UKWM_AUTORAISE_KEY));

    str = g_settings_get_string (ukwm_settings, UKWM_BUTTON_LAYOUT_KEY);
    gtk_widget_set_sensitive (GTK_WIDGET (titlebar_layout_optionmenu),
                              g_strcmp0 (str, UKWM_BUTTON_LAYOUT_LEFT) == 0 ||
                              g_strcmp0 (str, UKWM_BUTTON_LAYOUT_RIGHT) == 0);
    g_free (str);
}

static void
ukwm_settings_changed_callback (GSettings *settings,
                                 const gchar *key,
                                 gpointer user_data)
{
    update_sensitivity ();
...
autoraise_delay_value_changed_callback (GtkWidget *slider,
                                        void      *data)
{
    g_settings_set_int (ukwm_settings,
...
double_click_titlebar_changed_callback (GtkWidget *optionmenu,
                                        void      *data)
{
    g_settings_set_enum (ukwm_settings, UKWM_DOUBLE_CLICK_TITLEBAR_KEY,
                                  ...
                                  void      *data)
{
    ...
    if (value == 0) {
        g_settings_set_string (ukwm_settings, UKWM_BUTTON_LAYOUT_KEY, UKWM_BUTTON_LAYOUT_RIGHT);
    }
    else if (value == 1) {
        g_settings_set_string (ukwm_settings, UKWM_BUTTON_LAYOUT_KEY, UKWM_BUTTON_LAYOUT_LEFT);
    }
}
                                  ...
                                  void      *data)
{
    ...
    if (active) {
        ...
        g_settings_set_string (ukwm_settings, UKWM_MOUSE_MODIFIER_KEY, value);
        g_free (value);
    }
}
...
set_alt_click_value ()
{
    ...
    mouse_move_modifier = g_settings_get_string (ukwm_settings, UKWM_MOUSE_MODIFIER_KEY);
                     ...
                     void      *data)
{
    ...
    gtk_widget_set_sensitive (dialog_win, g_strcmp0 (current_wm, WM_COMMON_UKWM) == 0);
}
...
main (int argc, char **argv)
{
    ...
    if (g_strcmp0 (current_wm, WM_COMMON_METACITY) == 0) {
        ukui_metacity_config_tool ();
        return 0;
    }

    if (g_strcmp0 (current_wm, WM_COMMON_UKWM) != 0) {
        wm_unsupported ();
        return 1;
    }

    ukwm_settings = g_settings_new (UKWM_SCHEMA);
    ...
    gtk_combo_box_set_active (GTK_COMBO_BOX (titlebar_layout_optionmenu),
                              g_strcmp0 (str, UKWM_BUTTON_LAYOUT_RIGHT) == 0 ? 0 : 1);
    g_free (str);
    ...
    gtk_combo_box_set_active (GTK_COMBO_BOX (double_click_titlebar_optionmenu),
                              g_settings_get_enum (ukwm_settings, UKWM_DOUBLE_CLICK_TITLEBAR_KEY));

    set_alt_click_value ();
    gtk_range_set_value (GTK_RANGE (autoraise_delay_slider),
                         g_settings_get_int (ukwm_settings, UKWM_AUTORAISE_DELAY_KEY) / 1000.0);
    gtk_combo_box_set_active (GTK_COMBO_BOX (double_click_titlebar_optionmenu),
                              g_settings_get_enum (ukwm_settings, UKWM_DOUBLE_CLICK_TITLEBAR_KEY));
    ...
    g_signal_connect (ukwm_settings, "changed",
                      G_CALLBACK (ukwm_settings_changed_callback), NULL);

    g_settings_bind (ukwm_settings,
    ...
    g_settings_bind (ukwm_settings,
                     UKWM_SIDE_BY_SIDE_TILING_KEY,
                     side_by_side_tiling_checkbutton,
                     "active",
                     G_SETTINGS_BIND_DEFAULT);

    g_settings_bind (ukwm_settings,
                     UKWM_CENTER_NEW_WINDOWS_KEY,
                     center_new_windows_checkbutton,
                     "active",
                     G_SETTINGS_BIND_DEFAULT);

    g_signal_connect (ukwm_settings, "changed::" UKWM_FOCUS_KEY,
                      G_CALLBACK (mouse_focus_changed_callback), NULL);
    /* Initialize the checkbox state appropriately */
    mouse_focus_changed_callback(ukwm_settings, UKWM_FOCUS_KEY, NULL);
