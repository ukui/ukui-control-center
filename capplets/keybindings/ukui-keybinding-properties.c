...
create_builder (void)
{
  ...
  GError *error = NULL;
  static const gchar *uifile = UKUICC_UI_DIR "/ukui-keybinding-properties.ui";
...
reload_key_entries (GtkBuilder *builder)
{
  ...
  dir = g_dir_open (UKUICC_KEYBINDINGS_DIR, 0, NULL);
  if (!dir)
      return;
  ...
  for (l = list; l != NULL; l = l->next)
    ...
    path = g_build_filename (UKUICC_KEYBINDINGS_DIR, l->data, NULL);
...
static void
setup_dialog (GtkBuilder *builder, GSettings *ukwm_settings)
{
...
#if GTK_CHECK_VERSION(3, 0, 0)
  widget = _gtk_builder_get_widget (builder, "ukui-keybinding-dialog");
