/* ukui-theme-info.c - UKUI Theme information
 *
...
#define UKWM_THEME_KEY "X-GNOME-Metatheme/MetacityTheme"
 ...
 * UkuiTheme{Meta,Icon,}Info struct.  Note that the uri is of the form
 ...
 * by them is a GList whose data elements are UkuiTheme{Meta,Icon,}Info
                           ...
                           gpointer    data)
{
  ...
  if (list == NULL) {
    ...
    while (list_ptr) {
      ...
      theme_priority = ((UkuiThemeCommonInfo *) list_ptr->data)->priority;
                             ...
                             gint         priority)
{
  ...
  while (list) {
    UkuiThemeCommonInfo *info = (UkuiThemeCommonInfo *) list->data;
...
static gint
theme_compare (UkuiThemeCommonInfo *a,
               UkuiThemeCommonInfo *b)
{
  gint cmp;
  ...
  switch (a->type) {
  case UKUI_THEME_TYPE_METATHEME:
    cmp = ukui_theme_meta_info_compare (
                    (UkuiThemeMetaInfo *) a, (UkuiThemeMetaInfo *) b);
    break;
  case UKUI_THEME_TYPE_ICON:
    cmp = ukui_theme_icon_info_compare (
                    (UkuiThemeIconInfo *) a, (UkuiThemeIconInfo *) b);
    break;
  case UKUI_THEME_TYPE_CURSOR:
    cmp = ukui_theme_cursor_info_compare (
                    (UkuiThemeCursorInfo *) a, (UkuiThemeCursorInfo *) b);
    break;
  default:
...
static void
theme_free (UkuiThemeCommonInfo *info)
{
  switch (info->type) {
  case UKUI_THEME_TYPE_METATHEME:
    ukui_theme_meta_info_free ((UkuiThemeMetaInfo *) info);
    break;
  case UKUI_THEME_TYPE_ICON:
    ukui_theme_icon_info_free ((UkuiThemeIconInfo *) info);
    break;
  case UKUI_THEME_TYPE_REGULAR:
    ukui_theme_info_free ((UkuiThemeInfo *) info);
    break;
  case UKUI_THEME_TYPE_CURSOR:
    ukui_theme_cursor_info_free ((UkuiThemeCursorInfo *) info);
    break;
  default:
    g_assert_not_reached ();
  }
...
{
	return g_quark_from_static_string("ukui-theme-info-error-quark");
}
...
UkuiThemeMetaInfo* ukui_theme_read_meta_theme(GFile* meta_theme_uri)
{
	...
	meta_theme_info = ukui_theme_meta_info_new();
...
{
  UkuiThemeCursorInfo *theme_info;

  theme_info = ukui_theme_cursor_info_new ();
...
static UkuiThemeCursorInfo *
read_cursor_theme (GFile *cursor_theme_uri)
{
  UkuiThemeCursorInfo *cursor_theme_info = NULL;
  ...
  if (get_file_type (cursors_uri) == G_FILE_TYPE_DIRECTORY) {
    ...
    if (sizes->len == 0) {
      ...
      cursor_theme_info = ukui_theme_cursor_info_new ();
...
read_current_cursor_font (void)
{
  ...
  struct dirent *file_dirent;
	...
	dir_name = g_build_filename(g_get_user_config_dir(), "ukui", "share", "cursor-fonts", NULL);
...
read_cursor_fonts (void)
{
  ...
  const gchar *builtins[][4] = {
    {
      "ukui/cursor-fonts/cursor-normal.pcf",
      N_("Default Pointer"),
      N_("Default Pointer - Current"),
      "mouse-cursor-normal.png"
    }, {
      "ukui/cursor-fonts/cursor-white.pcf",
      N_("White Pointer"),
      N_("White Pointer - Current"),
      "mouse-cursor-white.png"
    }, {
      "ukui/cursor-fonts/cursor-large.pcf",
      N_("Large Pointer"),
      N_("Large Pointer - Current"),
      "mouse-cursor-normal-large.png"
    }, {
      "ukui/cursor-fonts/cursor-large-white.pcf",
  ...
  for (i = 0; i < G_N_ELEMENTS (builtins); i++) {
    ...
    filename = g_build_filename (UKUICC_DATA_DIR, "pixmaps", builtins[i][3], NULL);
    theme_info->thumbnail = gdk_pixbuf_new_from_file (filename, NULL);
    g_free (filename);

    theme_info->path = g_build_filename (UKUICC_DATA_DIR, builtins[i][0], NULL);
                      ...
                      UkuiThemeElement    element_type)
{
...
#endif
  UkuiThemeCommonInfo *theme = data;
  GList *list;
  ...
  if (theme->type == UKUI_THEME_TYPE_METATHEME)
    ...
    type_str = "cursor";
  else if (theme->type == UKUI_THEME_TYPE_REGULAR) {
    if (element_type & UKUI_THEME_GTK_2)
      element_str = "gtk-2";
    else if (element_type & UKUI_THEME_GTK_2_KEYBINDING)
      element_str = "keybinding";
    else if (element_type & UKUI_THEME_UKWM)
      element_str = "ukwm";
  }
  ...
  if (type == UKUI_THEME_TYPE_REGULAR) {
...
update_theme_index (GFile            *index_uri,
                    UkuiThemeElement key_element,
                    gint              priority)
{
  gboolean theme_exists;
  UkuiThemeInfo *theme_info;
  GFile *parent;
  GFile *common_theme_dir_uri;
  ...
  if (theme_info == NULL) {
    if (theme_exists) {
      theme_info = ukui_theme_info_new ();
      ...
      if (key_element & UKUI_THEME_GTK_2)
        theme_info->has_gtk = TRUE;
      else if (key_element & UKUI_THEME_GTK_2_KEYBINDING)
        theme_info->has_keybinding = TRUE;
      else if (key_element & UKUI_THEME_UKWM)
        theme_info->has_ukwm = TRUE;
      ...
      handle_change_signal (theme_info, UKUI_THEME_CHANGE_CREATED, key_element);
    }
  } else {
    ...
    if (key_element & UKUI_THEME_GTK_2) {
      ...
      theme_used_to_exist = theme_info->has_ukwm;
      theme_info->has_ukwm = theme_exists;
    }

    if (!theme_info->has_ukwm && !theme_info->has_keybinding && !theme_info->has_gtk) {
    ...
    if (theme_exists && theme_used_to_exist) {
      handle_change_signal (theme_info, UKUI_THEME_CHANGE_CHANGED, key_element);
    } else if (theme_exists && !theme_used_to_exist) {
      handle_change_signal (theme_info, UKUI_THEME_CHANGE_CREATED, key_element);
    } else if (!theme_exists && theme_used_to_exist) {
      handle_change_signal (theme_info, UKUI_THEME_CHANGE_DELETED, key_element);
    }

    if (!theme_info->has_ukwm && !theme_info->has_keybinding && !theme_info->has_gtk) {
      ukui_theme_info_free (theme_info);
    }
  }
...
update_gtk2_index (GFile *gtk2_index_uri,
                   gint   priority)
{
  update_theme_index (gtk2_index_uri, UKUI_THEME_GTK_2, priority);
}
...
update_keybinding_index (GFile *keybinding_index_uri,
                         gint   priority)
{
  update_theme_index (keybinding_index_uri, UKUI_THEME_GTK_2_KEYBINDING, priority);
}

static void
update_ukwm_index (GFile *ukwm_index_uri,
                       gint   priority)
{
  update_theme_index (ukwm_index_uri, UKUI_THEME_UKWM, priority);
}
...
{
  gboolean theme_exists;
  UkuiThemeCommonInfo *theme_info = NULL;
  UkuiThemeCommonInfo *old_theme_info;
  GFile *common_theme_dir_uri;
  gchar *common_theme_dir;
  GHashTable *hash_by_uri;
  GHashTable *hash_by_name;

  if (type == UKUI_THEME_TYPE_ICON) {
    hash_by_uri = icon_theme_hash_by_uri;
    hash_by_name = icon_theme_hash_by_name;
  } else if (type == UKUI_THEME_TYPE_CURSOR) {
  ...
  if (type != UKUI_THEME_TYPE_CURSOR) {
    ...
    if (get_file_type (theme_index_uri) == G_FILE_TYPE_REGULAR) {
      ...
      if (type == UKUI_THEME_TYPE_ICON)
        theme_info = (UkuiThemeCommonInfo *) read_icon_theme (theme_index_uri);
      else
        theme_info = (UkuiThemeCommonInfo *) ukui_theme_read_meta_theme (theme_index_uri);
    } else {
      theme_info = NULL;
    }
  ...
  /* cursor themes don't necessarily have an index file, so try those in any case */
    ...
    theme_info = (UkuiThemeCommonInfo *) read_cursor_theme (theme_index_uri);
  }
#endif
  ...
  old_theme_info = (UkuiThemeCommonInfo *) g_hash_table_lookup (hash_by_uri, common_theme_dir);
  ...
  if (old_theme_info == NULL) {
    if (theme_exists) {
      ...
      handle_change_signal (theme_info, UKUI_THEME_CHANGE_CREATED, 0);
    }
  } else {
    if (theme_exists) {
      if (theme_compare (theme_info, old_theme_info) != 0) {
        ...
        g_hash_table_insert (hash_by_uri, g_strdup (common_theme_dir), theme_info);
        add_theme_to_hash_by_name (hash_by_name, theme_info);
        handle_change_signal (theme_info, UKUI_THEME_CHANGE_CHANGED, 0);
      ...
      handle_change_signal (old_theme_info, UKUI_THEME_CHANGE_DELETED, 0);
      theme_free (old_theme_info);
    }
  }
...
update_meta_theme_index (GFile *meta_theme_index_uri,
                         gint   priority)
{
  update_common_theme_dir_index (meta_theme_index_uri, UKUI_THEME_TYPE_METATHEME, priority);
}
...
update_icon_theme_index (GFile *icon_theme_index_uri,
                         gint   priority)
{
  update_common_theme_dir_index (icon_theme_index_uri, UKUI_THEME_TYPE_ICON, priority);
}
...
{
#ifdef HAVE_XCURSOR
  update_common_theme_dir_index (cursor_theme_index_uri, UKUI_THEME_TYPE_CURSOR, priority);
#endif
}
...
static void
ukwm_dir_changed (GFileMonitor              *monitor,
                      GFile                     *file,
                      GFile                     *other_file,
                      GFileMonitorEvent          event_type,
                      CommonThemeDirMonitorData *monitor_data)
{
  ...
  if (!strcmp (affected_file, "metacity-theme-1.xml") || !strcmp (affected_file, "metacity-theme-2.xml")) {
    update_ukwm_index (file, monitor_data->priority);
  }
                              ...
                              GError                    **error)
{
  ...
  if (g_file_query_exists (uri, NULL)) {
    update_ukwm_index (uri, monitor_data->priority);
  }
  else {
    g_object_unref (uri);
  ...
  if (monitor != NULL) {
    g_signal_connect (monitor, "changed",
                      (GCallback) ukwm_dir_changed, monitor_data);
  }
  monitor_data->ukwm_dir_handle = monitor;
  g_object_unref (subdir);
...
remove_common_theme_dir_monitor (CommonThemeDirMonitorData *monitor_data)
{
  ...
  g_file_monitor_cancel (monitor_data->ukwm_dir_handle);
}
...
UkuiThemeInfo *
ukui_theme_info_new (void)
{
  UkuiThemeInfo *theme_info;

  theme_info = g_new0 (UkuiThemeInfo, 1);
  theme_info->type = UKUI_THEME_TYPE_REGULAR;

  return theme_info;
}

void
ukui_theme_info_free (UkuiThemeInfo *theme_info)
{
...
UkuiThemeInfo *
ukui_theme_info_find (const gchar *theme_name)
{
  return (UkuiThemeInfo *)
         get_theme_from_hash_by_name (theme_hash_by_name, theme_name, -1);
}

struct UkuiThemeInfoHashData
{
...
static void
ukui_theme_info_find_by_type_helper (gpointer key,
                                      GList *list,
                                      struct UkuiThemeInfoHashData *hash_data)
{
  guint elements = GPOINTER_TO_INT (hash_data->user_data);

  do {
    UkuiThemeInfo *theme_info = list->data;

    if ((elements & UKUI_THEME_UKWM && theme_info->has_ukwm) ||
        (elements & UKUI_THEME_GTK_2 && theme_info->has_gtk) ||
        (elements & UKUI_THEME_GTK_2_KEYBINDING && theme_info->has_keybinding)) {
...
GList *
ukui_theme_info_find_by_type (guint elements)
{
  struct UkuiThemeInfoHashData data;
  data.user_data = GINT_TO_POINTER (elements);
  data.list = NULL;

  g_hash_table_foreach (theme_hash_by_name,
                        (GHFunc) ukui_theme_info_find_by_type_helper,
                        &data);

  return data.list;
}

static void ukui_theme_info_find_all_helper(const gchar* key, GList* list, GList** themes)
{
	/* only return visible themes */
	if (!((UkuiThemeCommonInfo*) list->data)->hidden)
	{
...
/* Icon themes */
UkuiThemeIconInfo *
ukui_theme_icon_info_new (void)
{
  UkuiThemeIconInfo *icon_theme_info;

  icon_theme_info = g_new0 (UkuiThemeIconInfo, 1);
  icon_theme_info->type = UKUI_THEME_TYPE_ICON;

  return icon_theme_info;
}

void
ukui_theme_icon_info_free (UkuiThemeIconInfo *icon_theme_info)
{
...
gint
ukui_theme_icon_info_compare (UkuiThemeIconInfo *a,
                               UkuiThemeIconInfo *b)
{
  gint cmp;
...
/* Cursor themes */
UkuiThemeCursorInfo *
ukui_theme_cursor_info_new (void)
{
  UkuiThemeCursorInfo *theme_info;

  theme_info = g_new0 (UkuiThemeCursorInfo, 1);
  theme_info->type = UKUI_THEME_TYPE_CURSOR;

  return theme_info;
}

void
ukui_theme_cursor_info_free (UkuiThemeCursorInfo *cursor_theme_info)
{
...
gint
ukui_theme_cursor_info_compare (UkuiThemeCursorInfo *a,
...
UkuiThemeMetaInfo* ukui_theme_meta_info_new(void)
{
	...
	theme_info = g_new0(UkuiThemeMetaInfo, 1);
	theme_info->type = UKUI_THEME_TYPE_METATHEME;
...
void ukui_theme_meta_info_free(UkuiThemeMetaInfo* meta_theme_info)
{
	...
	g_free(meta_theme_info->ukwm_theme_name);
	g_free(meta_theme_info->notification_theme_name);
	g_free(meta_theme_info);
}

gboolean ukui_theme_meta_info_validate(const UkuiThemeMetaInfo* info, GError** error)
{
	UkuiThemeInfo* theme;

	g_return_val_if_fail (error == NULL || *error == NULL, FALSE);

	theme = ukui_theme_info_find (info->gtk_theme_name);

	if (!theme || !theme->has_gtk)
	{
		g_set_error (error, UKUI_THEME_ERROR, UKUI_THEME_ERROR_GTK_THEME_NOT_AVAILABLE,
			_("This theme will not look as intended because the required GTK+ theme '%s' is not installed."),
			info->gtk_theme_name);
		return FALSE;
	}

	theme = ukui_theme_info_find (info->ukwm_theme_name);

	if (!theme || !theme->has_ukwm)
	{
		g_set_error (error, UKUI_THEME_ERROR, UKUI_THEME_ERROR_WM_THEME_NOT_AVAILABLE,
			_("This theme will not look as intended because the required window manager theme '%s' is not installed."),
			info->ukwm_theme_name);
		return FALSE;
	}

	if (!ukui_theme_icon_info_find (info->icon_theme_name))
	{
		g_set_error (error, UKUI_THEME_ERROR, UKUI_THEME_ERROR_ICON_THEME_NOT_AVAILABLE,
	...
	if (engine != NULL)
	{
		g_set_error (error, UKUI_THEME_ERROR, UKUI_THEME_ERROR_GTK_ENGINE_NOT_AVAILABLE,
...
UkuiThemeMetaInfo* ukui_theme_meta_info_find(const char* meta_theme_name)
{
	g_return_val_if_fail(meta_theme_name != NULL, NULL);

	return (UkuiThemeMetaInfo*) get_theme_from_hash_by_name (meta_theme_hash_by_name, meta_theme_name, -1);
}
...
GList* ukui_theme_meta_info_find_all(void)
{
  ...
  g_hash_table_foreach (meta_theme_hash_by_name, (GHFunc) ukui_theme_info_find_all_helper, &list);

  return list;
}

gint
ukui_theme_meta_info_compare (UkuiThemeMetaInfo *a,
                               UkuiThemeMetaInfo *b)
{
  gint cmp;
  ...
  cmp = safe_strcmp (a->ukwm_theme_name, b->ukwm_theme_name);
  if (cmp != 0) return cmp;
...
void
ukui_theme_info_register_theme_change (ThemeChangedCallback func,
                                        gpointer data)
{
...
gboolean
#if GTK_CHECK_VERSION (3, 0, 0)
ukui_theme_color_scheme_parse (const gchar *scheme, GdkRGBA *colors)
#else
ukui_theme_color_scheme_parse (const gchar *scheme, GdkColor *colors)
#endif
{
...
gboolean
ukui_theme_color_scheme_equal (const gchar *s1, const gchar *s2)
{
#if GTK_CHECK_VERSION (3, 0, 0)
  ...
  if (!ukui_theme_color_scheme_parse (s1, c1) ||
      !ukui_theme_color_scheme_parse (s2, c2))
    return FALSE;
...
ukui_theme_init ()
{
  ...
  if (!ukui_theme_cursor_info_find ("default"))
    add_default_cursor_theme ();
#else
