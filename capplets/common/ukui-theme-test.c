...
main (int argc, char *argv[])
{
  ...
  themes = ukui_theme_meta_info_find_all ();
  if (themes == NULL)
    {
  ...
  if (themes == NULL)
    {
      g_print ("No ukwm themes were found.\n");
    }
  else
    {
      g_print ("%d ukwm themes were found:\n", g_list_length (themes));
      for (list = themes; list; list = list->next)
	{
  ...
  themes = ukui_theme_info_find_by_type (UKUI_THEME_GTK_2_KEYBINDING);
  if (themes == NULL)
    {
  ...
  if (themes == NULL)
  ...
  else
    {
      ...
      for (list = themes; list; list = list->next)
	{
	  UkuiThemeCursorInfo *cursor_theme_info;
