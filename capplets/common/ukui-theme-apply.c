 ...
 * themus - utilities for UKUI themes
...
void
ukui_meta_theme_set (UkuiThemeMetaInfo *meta_theme_info)
{
  ...
  if (compare (old_key, meta_theme_info->gtk_color_scheme))
    {
      ...
      if (newval == NULL || !strcmp (newval, "") ||
          ukui_theme_color_scheme_equal (newval, gtkcols))
        {
  ...
  /* Set the wm key */
  g_settings_set_string (ukwm_settings, UKWM_THEME_KEY, meta_theme_info->ukwm_theme_name);
