...
#define A11Y_SCHEMA "org.ukui.accessibility-keyboard"
...
bell_flash_radio_changed (GtkWidget *widget, GtkBuilder *builder)
{
	...
	int old_bell_flash_type = g_settings_get_enum (ukwm_settings, "visual-bell-type");
	...
	if (old_bell_flash_type != new_bell_flash_type)
		g_settings_set_enum (ukwm_settings, "visual-bell-type", new_bell_flash_type);
...
notifications_button_clicked_cb (GtkWidget *button, GtkBuilder *dialog)
{
    ...
    gtk_builder_add_from_file (notifications_dialog, UKUICC_UI_DIR
                               "/ukui-keyboard-properties-a11y-notifications.ui",
                               NULL);
	...
	GSettings *ukwm_settings = g_settings_new (UKWM_SCHEMA);
	w = NWID ("visual_bell_enable");
	g_settings_bind (ukwm_settings, "visual-bell", w, "active", G_SETTINGS_BIND_DEFAULT);
	g_signal_connect (w, "toggled",
			G_CALLBACK (visual_bell_enable_toggled_cb), dialog);
	visual_bell_enable_toggled_cb (w, dialog);

	bell_flash_gsettings_changed (ukwm_settings, "visual-bell-type", NULL);
	...
	g_signal_connect (NWID ("visual_bell_fullscreen"), "clicked",
					  G_CALLBACK(bell_flash_radio_changed),
					  notifications_dialog);
	g_signal_connect (ukwm_settings,
