...
//    widget->setFixedHeight(56);
    ...
    QPixmap pixmap=loadSvg(QString("://img/dropArrow/ukui-control center-tab selected.svg"),"default");
    arrows = new QLabel(widget);
    arrows->setStyleSheet("background:transparent");
    arrows->setFixedSize(pixmap.size());
    arrows->setPixmap(pixmap);
...
void LeftWidgetItem::setSelected(bool selected){
    is_seletced = selected;
    if (selected) {
        arrows->setVisible(true);
        widget->setStyleSheet("QWidget{background: #2FB3E8; border-radius: 8px;}");
    } else {
        arrows->setVisible(false);
//        widget->setStyleSheet("QListWidget::Item:hover{background:transparent;border-radius: 8px;}");
        widget->setStyleSheet("QWidget{background: transparent; border-radius: 8px;}");
    }
}
