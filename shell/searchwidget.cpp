...
void SearchWidget::hasAutoLight()
{
    bool hasAutoLighted;
    ...
    if (hasAutoLighted) {
        ...
        isHasAutoLight = false;
    }
}

void SearchWidget::screenAddedProcess() {
    screenNum = QGuiApplication::screens().count();
    loadxml();
}
...
void SearchWidget::loadxml() {
    ...
    for (const QString i : m_xmlFilePath) {
        ...
        QString monitorFile = homePath+"/.config/search.xml";
        //qDebug()<<monitorFile<<endl;
        QFile searchfile(monitorFile);
        if (!searchfile.open(QIODevice::WriteOnly | QIODevice::Text)){
            qDebug()<<"open file failed"<<endl;
            return ;
        }
        xmlWriter = new QXmlStreamWriter(&searchfile);
                    ...
                    if (m_xmlExplain == XML_Source) { // get xml source date
                        ...
                        if (!m_bIsChinese) {
                            ...
                            for (auto it : m_moduleNameList) {
                                if(it.second == m_searchBoxStruct.actualModuleName) {
                                    xmlWriter->writeAttribute("name",it.first);
                                    break;
                                }
                            }
                            appendChineseData(m_searchBoxStruct);
//                            qDebug()<<m_searchBoxStruct.actualModuleName<<" "<<m_searchBoxStruct.translateContent;
                            xmlWriter->writeTextElement("ChinesePlugin",m_searchBoxStruct.actualModuleName);
                            xmlWriter->writeTextElement("ChineseFunc",m_searchBoxStruct.translateContent);
...
void SearchWidget::appendChineseData(SearchWidget::SearchBoxStruct data) {
    if ("" == data.childPageName) {
        ...
        pinyinTxt = QString("%1 --> %2")
                            .arg(removeDigital(Chinese2Pinyin(data.actualModuleName)))
                            .arg(removeDigital(Chinese2Pinyin(data.translateContent)));
//        qDebug()<<removeDigital(Chinese2Pinyin(data.actualModuleName))<<removeDigital(Chinese2Pinyin(data.translateContent));

        xmlWriter->writeTextElement("pinyinPlugin",removeDigital(Chinese2Pinyin(data.actualModuleName)));
        xmlWriter->writeTextElement("pinyinfunc",removeDigital(Chinese2Pinyin(data.translateContent)));

        //添加显示的汉字(用于拼音搜索显示)
        m_model->appendRow(new QStandardItem(/*icon.value(),*/ hanziTxt));
        if (screenNum == 1 && pinyinTxt.contains("pingmu --> pingmujingxiang")) {
            pinyinTxt.remove("pingmu --> pingmujingxiang");
        }
        pinyinTxt.remove("pingmu --> zidongdiaozhengliangdu");
